...
def swap_ativos_entre_bases(solution, dist_bases_ativos):
    # Copiar a solução atual para não modificar diretamente a original
    new_solution = copy.deepcopy(solution)

    # 1. Selecionar equipe1 para swap 
    ...
    if isSwap:
        ...
        for ativo in ativos_equipe2:
            ...
            if (np.argmin(distancias) != equipe2_para_swap):# and (np.argmin(distancias) == equipe1_para_swap):
                equipe3_para_swap = np.argmin(distancias)
                base_equipe2 = bases_equipes[equipe2_para_swap]
                base_equipe3 = bases_equipes[equipe3_para_swap]
                #print("SWAP2:\n")
        
                # Trocar ativo de equipe1 para equipe2
                new_solution['h'][ativo,equipe2_para_swap] = 0
                new_solution['h'][ativo,equipe3_para_swap] = 1

                # Trocar ativo de base de equipe1 para base de equipe2
                new_solution['x'][ativo,base_equipe2] = 0
                new_solution['x'][ativo,base_equipe3] = 1
                ...
                if(np.sum(new_solution['h'][:, equipe1_para_swap],axis=0) == np.sum(new_solution['x'][:, base_equipe1],axis=0)):
                    if np.sum(new_solution['h'][:, equipe1_para_swap],axis=0) < 30:
                        print("ERRO DE CONGRUÊNCIA DE DADOS!")
                        exit()
                break
    return new_solution
    
def trocar_ativos_entre_bases2(solution, dist_bases_ativos):
...
def trocar_equipes_de_bases2(solution, dist_bases_ativos):
...
def trocar_equipes_de_bases_f2(solution,dist_bases_ativos,prob_ativos):
    # Copiar a solução atual para não modificar diretamente a original
    new_solution = copy.deepcopy(solution)
    ...
    # Gerar distâncias ponderadas:
    dist_bases_ativos_risco = np.zeros((num_ativos,num_bases))
    for ativo in range(num_ativos):
        for base in range(num_bases):
            dist_bases_ativos_risco[ativo,base] = dist_bases_ativos[ativo,base]*prob_ativos[ativo,2]
    ...
    # selecionar uma base aleatoria (sem equipe alocada) dentre as seis com menor soma de distâncias PONDERADAS
    soma_distancias_bases = []
    for base_disponivel in bases_disponiveis:
        # Ordenar as distâncias dos 15 ativos para a base atual
        distancias_ordenadas = np.sort(dist_bases_ativos_risco[:15, base_disponivel])
        # Somar as distâncias dos ativos mais próximos
        soma_distancias = np.sum(distancias_ordenadas[:15])
        soma_distancias_bases.append((base_disponivel, soma_distancias))
    ...
    if len(equipe_associada) > 0:
        ...
        distancias_bases_ocupadas = dist_bases_ativos_risco[:, bases_com_equipes]
        # Adicionar o index do ativo na coluna
        rows_indices = np.arange(distancias_bases_ocupadas.shape[0], dtype=int).reshape(-1, 1)
        dist_bases_ocupadas_com_index = np.hstack((rows_indices, distancias_bases_ocupadas))
...
def swap_ativos_bases_f2(solution,dist_bases_ativos,prob_ativos):
    # Copiar a solução atual para não modificar diretamente a original
    new_solution = copy.deepcopy(solution)
    ...
    distancias_ponderadas_risco = np.zeros((num_ativos, num_equipes))
    # 4. Obter matriz de distâncias * probabilidade de erro
    for ativo in range(num_ativos):
        for index, base_equipe in enumerate(bases_equipes):
            distancias_ponderadas_risco[ativo,index] = dist_bases_ativos[ativo,base_equipe] * prob_ativos[ativo,2]

    # 5. Percorrer ativos de equipe1 e comparar distâncias a outras bases de outras equipes
    isSwap = False
    equipe2_para_swap = 0
    for ativo in ativos_equipe1:
        distancia_ponderada_ativo = distancias_ponderadas_risco[ativo,:]
        
        if np.argmin(distancia_ponderada_ativo) != equipe1_para_swap:
            isSwap = True
            #print("SWAP1:\n")
            equipe2_para_swap = np.argmin(distancia_ponderada_ativo)
            base_equipe1 = bases_equipes[equipe1_para_swap]
            base_equipe2 = bases_equipes[equipe2_para_swap]
    ...
    if isSwap:
        ...
        for ativo in ativos_equipe2:
            ...
            if (np.argmin(distancia_ponderada_ativo) != equipe2_para_swap):# and (np.argmin(distancias) == equipe1_para_swap):
                equipe3_para_swap = np.argmin(distancia_ponderada_ativo)
                base_equipe2 = bases_equipes[equipe2_para_swap]
                base_equipe3 = bases_equipes[equipe3_para_swap]
                #print("SWAP2:\n")
...
def neighborhood_change(solution,obj_function,neighborhood,dist_bases_ativos,prob_ativos):
    ...
    if obj_function == 1:
        match neighborhood:
            ...
            case 3:
                return trocar_ativos_entre_bases2(solution,dist_bases_ativos)
